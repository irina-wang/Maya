...
class camDockableWidget(MayaQWidgetDockableMixin, QWidget):
    ...
    #             widget.deleteLater()
    #     for light in pm.ls(tsype=["areaLight", "spotLight", "pointLight", "directionalLight", "volumeLight"]):
    #         self.addLight(light)
    ...
    def displayGrpTab(self, grpName, mainObj):
        ...
        # intensity = QtWidgets.QSlider(Qt.Horizontal)
        # intensity.setMinimum(1)
        # intensity.setMaximum(1000)
        # intensity.setValue(self.keylight.intensity.get())
        # intensity.valueChanged.connect(lambda val: self.keylight.intensity.set(val))
        # panelLayout.addWidget(intensity, 1, 0, 1, 2)

        # offset = QtWidgets.QInputContext()
        # offset.setMinimum(1)
        # offset.setMaximum(1000)
        # offset.setValue(self.offset.get())
        # offset.valueChanged.connect(lambda val: self.keylight.offset.set(val)) 
        # panelLayout.addWidget(offset, 2, 0, 2, 2)
    ...
    # def changeLR(self, val):
    #     # self.keylightlr = val
    #     self.keylight.rotate.set([-val, val, 0])
